 ...
 *  If the passed pointer points to memory you are not allowed to access and you redeference it, the probram will
 *  probably crash.
 *
 * @param args
...
declfn void instance::start( _In_opt_ const args_t* args ) {
  ...
  if ( args && args->length > 0 ) {
    win32::call< fnMessageBoxA >( H_FUNC( "user32!MessageBoxA" ), nullptr, "HackThePlanet", args->arguments[ 0 ],
        MB_OK );
  } else {
